...
migrate((app) => {
  const journals = app.findAllRecords("journals")
  ...
  const posts = app.findRecordsByFilter("posts", 'type = "journal"')
  const postFlatsColl = app.findCollectionByNameOrId("post_flats")
  ...
  posts.forEach((record) => {
    ...
    // create related post_flats records
    
    app.expandRecord(record, ["journal"], null)
    const journalRecord = record.expandedOne("journal")

    const postFlatRecord = new Record(postFlatsColl, {
      post: record.id,
      flat: journalRecord.get("flat")
    })

    app.save(postFlatRecord)
  })
}, (app) => {
  const journals = app.findRecordsByFilter("posts", 'type = "journal"')
